...
import com.imperialm.imimserservices.dto.MyfcaMSERTotalEarningsDetailsDTO;
import com.imperialm.imimserservices.model.response.TotalName;
...
public interface MyfcaMSERTotalEarningsDetailsDAO {
	...
	public static String EXCELLENCE_CARD_AWARDS_YTD_NAT = "SELECT CAST(SUM([EarningsYTD]) as varchar(20)) 'total', 'Excellence Card Awards YTD' as name, '' as error FROM [MyfcaMSERTotalEarningsDetails]";
	public static String EXCELLENCE_CARD_AWARDS_MTD_NAT = "SELECT CAST(SUM([EarningsMTD]) as varchar(20)) 'total', 'Excellence Card Awards MTD' as name, '' as error FROM [MyfcaMSERTotalEarningsDetails]";
	//Add group by
	...
	public static String SELECT_PARTICIPANT_ENROLLED_BY_DEALERCODE = "SELECT CAST(ISNULL(COUNT(DISTINCT SID),0) as varchar(20)) total , 'Total Participants Enrolled' name, '' as error FROM [MyfcaMSERTotalEarningsDetails] where ParticipantsEnrolled > 0 AND DealerCode = ?0";
	public static String SELECT_TOTAL_EARNINGS_YTD_BY_BC_OR_DISTRICT = "Select IsNull(CAST(SUM([Amount]) as varchar(20)),'0') 'total', 'Excellence Card Awards YTD' as 'name', '' as error From (SELECT SUM([Amount]) Amount FROM [MyfcaMSERTotalEarnings] where child = ?0 and toggle = 'ytd' group by Child) A";
	public static String SELECT_TOTAL_EARNINGS_MTD_BY_BC_OR_DISTRICT = "Select IsNull(CAST(SUM([Amount]) as varchar(20)),'0') 'total', 'Excellence Card Awards MTD' as 'name', '' as error From (SELECT SUM([Amount]) Amount FROM [MyfcaMSERTotalEarnings] where child = ?0 and toggle = 'mtd' group by Child) A";
	
	public static String SELECT_BY_DEALER_CODE = "SELECT [DealerCode] 'dealerCode' ,[DealerName] 'dealerName' ,[SID] 'sID' ,[Name] 'name' ,[PositionCode] 'positionCode' ,[Program] 'program' ,[ProgramGroup]'programGroup' ,[EarningsMTD] 'earningsMTD' ,[EarningsYTD] 'earningsYTD' ,[DealersEnrolled] 'dealersEnrolled' ,[ParticipantsEnrolled] 'participantsEnrolled', '' as error FROM [MyfcaMSERTotalEarningsDetails] where DealerCode = ?0";
	public static String SELECT_BY_SID = "SELECT [DealerCode] 'dealerCode' ,[DealerName] 'dealerName' ,[SID] 'sID' ,[Name] 'name' ,[PositionCode] 'positionCode' ,[Program] 'program' ,[ProgramGroup]'programGroup' ,[EarningsMTD] 'earningsMTD' ,[EarningsYTD] 'earningsYTD' ,[DealersEnrolled] 'dealersEnrolled' ,[ParticipantsEnrolled] 'participantsEnrolled', '' as error FROM [MyfcaMSERTotalEarningsDetails] where SID = ?0 and DealerCode = ?1";
	public static String SELECT_SUM_BY_SID = "SELECT [DealerCode] 'dealerCode' ,'' as 'dealerName' ,[SID] 'sID' ,'' as 'name' ,'' as 'positionCode' ,'' as 'program' ,'' as'programGroup' ,SUM([EarningsMTD]) 'earningsMTD' , SUM([EarningsYTD]) 'earningsYTD' , SUM([DealersEnrolled]) 'dealersEnrolled' , SUM([ParticipantsEnrolled]) 'participantsEnrolled', '' as error FROM [MyfcaMSERTotalEarningsDetails] where SID = ?0 and DealerCode = ?1 group by sid, DealerCode";
