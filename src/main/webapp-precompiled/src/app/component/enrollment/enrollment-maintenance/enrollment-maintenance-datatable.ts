...
@Component({
    moduleId: module.id,
    selector: 'enrollmentdatatable',
    template: ``
})

export class EnrollmentMaintenanceTableComponent implements OnChanges, OnDestroy {
    private otable: any;
    ...
    ngOnChanges(): void {
        ...
        if ($.fn.dataTable.isDataTable('enrollmentmaintenance' + this.randomId)) {
        ...
        var table = $('#enrollmentmaintenance' + this.randomId).DataTable({
            // 'bSort': false,
            "data": data,
            "columns": columns,
